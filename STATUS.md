...
- ~~**Smart ban Phases 1-2**~~: Fixed. Per-block SHA-1 attribution landed in commit `6b6688d` (`src/net/smart_ban.zig`). Integrated into `processHashResults` for fair peer banning in multi-source scenarios.
- **MSE handshake failures in mixed encryption mode**: `vc_not_found` and `req1_not_found` errors occur during simultaneous inbound+outbound MSE handshakes. Timing-dependent, disappears under GDB. `demo_swarm.sh` runs with `encryption = "preferred"` (works) but occasional stall has been observed in stress tests.
- **uTP BT send cannot span multiple uTP packets**: transfers larger than ~2 KB over uTP fail: peers complete MSE + extension handshake, but BITFIELD / PIECE messages that cross a uTP packet boundary never arrive, so `progress` stays at 0. TCP works correctly for the same files. Reproduce: `demo_swarm.sh` with a >3 KB payload, or `scripts/test_transfer_matrix.sh` with `enable_utp = true`. Workaround: `enable_utp = false` in daemon config (the transfer matrix test now sets this). Likely root cause is in the uTP send-side fragmentation or recv-side reassembly in `src/net/utp.zig` / `src/io/utp_handler.zig`.
- **Shared default `resume.db` conflates daemon instances**: the default resume DB path is `~/.local/share/varuna/resume.db`, shared across every daemon running as the user. Two daemons sharing it (e.g. a seeder and downloader in a test) see each other's completion records via shared `info_hash`, and the downloader's `skip_recheck` path treats pieces as already complete even when its on-disk data is zero. Set `resume_db` explicitly per daemon (all bundled scripts now do this).
- **`large-20m-64k` transfer intermittently stalls at `progress=0.0000`**: 4 of 5 consecutive `test_transfer_matrix.sh` runs complete 24/24, but ~20% of runs see `large-20m-64k` (20 MB payload, 64 KB pieces, 320 pieces × 4 blocks) time out with no data transferred despite peers being connected. Test infrastructure was ruled out: each test now runs in its own `127.0.${TEST_INDEX}.x /24` with graceful API shutdown between tests, so no kernel state persists across tests. Smaller torrents pass reliably, and the same config completes in ~6 s when run in isolation. The stall is in varuna's early peer-to-peer handshake under this specific layout. Follow-up: instrument the peer state machine on stall to capture which message is missing.
