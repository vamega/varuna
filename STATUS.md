...
- Torrent data relocation: async file-move job + new `POST /api/v2/varuna/torrents/move` (returns job id), `GET /move/<id>` for progress polling, `/cancel`, `/commit`, `DELETE` for cleanup. Same-fs detection via `posix.fstatat` `dev_t` ⇒ single `rename(2)` (constant-time on every modern Linux filesystem); cross-fs ⇒ recursive walk + `posix.copy_file_range` per file with 32 MiB chunk cap. Worker thread + atomic progress counters; `O_NOFOLLOW` symlink safety. The qBittorrent-compatible `setLocation` endpoint now returns 400 with a body that names the new endpoint (its synchronous semantics are incompatible with the daemon's performance guarantees). See `progress-reports/2026-04-30-async-move-job.md`.
...
- ~~**Daemon submission paths for bind / listen / setsockopt.**~~ Listener bring-up landed 2026-04-30. New contract ops `bind` / `listen` / `setsockopt` in `src/io/io_interface.zig` (`BindOp` / `ListenOp` / `SetsockoptOp`); `RealIO` impls branch on `feature_support.supports_bind` / `supports_listen` / `supports_setsockopt` — async via `IORING_OP_BIND` / `IORING_OP_LISTEN` / `IORING_OP_URING_CMD+SOCKET_URING_OP_SETSOCKOPT` when supported, synchronous `posix.bind/listen/setsockopt(2)` fallback otherwise. SimIO / EpollPosixIO / EpollMmapIO / KqueuePosixIO / KqueueMmapIO ship synchronous-success / synchronous-fallback impls. New `io_interface.bindBlocking` / `listenBlocking` / `setsockoptBlocking` helpers wrap the contract ops in a "submit + tick until done" pattern for one-shot startup paths; the three daemon listener bring-up paths (`event_loop.startUtpListener`, `event_loop.startTcpListener`, `rpc.server.ApiServer.initWithDevice`) now route through them. Per-peer `configurePeerSocket` (TCP_NODELAY, buffer sizes) and `applyBindDevice` are deliberately left synchronous — those callsites are fire-and-forget steady-state ops with no completion home, and the kernel processes them in-process anyway (no I/O wait, so async dispatch offers near-zero gain). `main.zig` startup port-scan stays synchronous (pre-event-loop, no IO instance available). See `progress-reports/2026-04-30-bind-listen-setsockopt.md`.
...
- **EL-driven MoveJob via IO-contract directory ops** (also unblocks EL-driven `PieceStore.init`). Partial groundwork landed 2026-05-01 and 2026-05-02: the IO contract now has `openat` / `mkdirat` / `renameat` / `unlinkat` / `statx` / `getdents` operation/result variants. RealIO submits native SQEs for the kernel-supported namespace ops (`openat`, `mkdirat`, `renameat`, `unlinkat`, `statx`) when the runtime probe reports support and falls back to sync syscalls otherwise; `getdents` is a Linux syscall-shaped synchronous contract op because Zig 0.15.2 exposes no io_uring getdents helper/op. SimIO has a deterministic virtual namespace plus fault knobs for all six ops, and the epoll/kqueue dev backends expose the same methods. MoveJob itself is still the thread-based v1 and still walks via `opendir` / `readdir` / `fstatat` / `rmdir` plus raw copy/unlink loops. Remaining blockers before an EL-driven MoveJob v2: decide the job scheduling policy (global cap vs per-device partitioning vs FIFO/size-aware ordering), then rewrite MoveJob as an EL-owned state machine using the directory ops plus existing `splice` / `copy_file_range`. See `progress-reports/2026-05-01-io-directory-ops.md` and `progress-reports/2026-05-02-io-directory-metadata.md`.
- **Review whether `io_interface.{bind,listen,setsockopt}Blocking` helpers should stay.** They wrap "submit + tick(1) until done" for one-shot startup-time callers (listener bring-up where there's no natural completion home). Worth a deliberate look down the line — keeping them is fine if the call sites stay startup-only, but if more steady-state code starts reaching for them, the right fix is usually to surface the async form and let the caller manage a real state machine. No urgency; bring-up paths are correct and tested.
