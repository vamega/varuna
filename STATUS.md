...
- ~~**`large-20m-64k` transfer intermittently stalls at `progress=0.0000`**~~: Fixed in commit `57f292d`. Root cause was `peer.address` left `undefined` on inbound TCP accepts (`peer_handler.zig:handleAccept`). Downstream callers (`isPeerAlreadyConnected` in PEX, periodic ban sweep, smart-ban ban-and-disconnect, `/sync/torrentPeers`) read the garbage field and occasionally mis-matched real connections, killing them. Fix resolves the remote address via `getpeername` up front and stores it in the Peer. See `progress-reports/2026-04-22-inbound-peer-address-fix.md`.
