...
- **`SimPeer` (`src/sim/sim_peer.zig`, Stage 4 #6 partial)**: scriptable BitTorrent seeder driven by SimIO completions. Plays the seeder side of the wire protocol (handshake → bitfield → unchoke → piece) over a SimIO socketpair. Behaviors: honest, slow (stub), corrupt, wrong_data, silent_after, disconnect_after, lie_bitfield. 8 unit tests in `tests/sim_peer_test.zig` exercise honest, wrong_data, corrupt, disconnect_after, lie_bitfield, and silent_after end-to-end against a manual test-side downloader. The full smart-ban sim test (5 honest + 1 corrupt vs. a real EventLoop) is gated on Stage 2 #11/#12 finishing.
