...
- **EL-driven MoveJob via IO-contract directory ops** (also unblocks EL-driven `PieceStore.init`). Partial groundwork landed 2026-05-01: the IO contract now has `openat` / `mkdirat` / `renameat` / `unlinkat` operation/result variants, RealIO submits native SQEs when the runtime probe reports support and falls back to `posix.*`, SimIO has a deterministic virtual namespace plus fault knobs for those four ops, and the epoll/kqueue dev backends have synchronous POSIX implementations. MoveJob itself is still the thread-based v1 and still walks via `opendir` / `readdir` / `fstatat` / `rmdir` plus raw copy/unlink loops. Remaining blockers before an EL-driven MoveJob v2: add `getdents` (or equivalent directory enumeration) and `statx`/metadata contract coverage, decide the job scheduling policy (global cap vs per-device partitioning vs FIFO/size-aware ordering), then rewrite MoveJob as an EL-owned state machine using the new directory ops plus existing `splice` / `copy_file_range`. See `progress-reports/2026-05-01-io-directory-ops.md`.
