...
- Torrent data relocation: async file-move job + new `POST /api/v2/varuna/torrents/move` (returns job id), `GET /move/<id>` for progress polling, `/cancel`, `/commit`, `DELETE` for cleanup. Production relocation is manifest-scoped and event-loop driven through the IO contract; same-fs moves use `renameat`, cross-fs moves use `openat` + `copy_file_chunk` + destination `fchown`/`fchmod` + `fsync` + `unlinkat`. RealIO implements the semantic copy session with a backend-owned pipe and private `IORING_OP_SPLICE` state machine; `IORING_OP_PIPE` is used when available and otherwise falls back to `pipe2(O_CLOEXEC|O_NONBLOCK)` with next-tick ready completion. POSIX and mmap readiness backends offload `copy_file_chunk` / metadata ops to backend-owned file-op pools. Atomic progress counters; `O_NOFOLLOW` symlink safety. Basic ownership and permission/special-bit metadata is preserved for copied files and created directories; timestamps, xattrs, and ACLs remain a feasibility/fidelity follow-up. The qBittorrent-compatible `setLocation` endpoint now returns 400 with a body that names the new endpoint (its synchronous semantics are incompatible with the daemon's performance guarantees). `copy_file_range` on a backend-owned threadpool remains a possible future experiment only if profiling finds a concrete reason. See `progress-reports/2026-04-30-async-move-job.md`, `progress-reports/2026-05-03-move-job-event-loop.md`, `progress-reports/2026-05-05-io-contract-bypass-cleanup.md`, and `progress-reports/2026-05-06-movejob-splice-fallback.md`.
...
- ~~**Daemon submission paths for bind / listen / setsockopt.**~~ Listener bring-up landed 2026-04-30; helper cleanup landed 2026-05-05. Contract ops `bind` / `listen` / `setsockopt` in `src/io/io_interface.zig` (`BindOp` / `ListenOp` / `SetsockoptOp`) remain the only IO-interface surface. `RealIO` impls branch on `feature_support.supports_bind` / `supports_listen` / `supports_setsockopt` — async via `IORING_OP_BIND` / `IORING_OP_LISTEN` / `IORING_OP_URING_CMD+SOCKET_URING_OP_SETSOCKOPT` when supported, synchronous `posix.bind/listen/setsockopt(2)` fallback otherwise. SimIO / EpollPosixIO / EpollMmapIO / KqueuePosixIO / KqueueMmapIO ship synchronous-success / synchronous-fallback impls. The former `io_interface.*Blocking` wrappers were removed because the IO contract should not advertise caller-blocking conveniences; one-shot listener startup now owns its local "submit + tick until done" wait loops in `event_loop.zig` and `rpc/server.zig`. Per-peer `configurePeerSocket` (TCP_NODELAY, buffer sizes) and `applyBindDevice` remain separate cleanup candidates. `main.zig` startup port-scan stays synchronous (pre-event-loop, no IO instance available). See `progress-reports/2026-04-30-bind-listen-setsockopt.md` and `progress-reports/2026-05-05-io-contract-bypass-cleanup.md`.
...
- ~~**EL-driven MoveJob via IO-contract directory ops**~~ Closed 2026-05-03; thread-path cleanup landed 2026-05-05; copy-session cleanup landed 2026-05-06. Production relocation is manifest-scoped and event-loop driven: `SessionManager.startMoveJob` constructs `MoveJob.createForFiles`, calls `MoveJob.startOnEventLoop`, and `SessionManager.tickMoveJobs` advances each active job on the shared event loop. The MoveJob state machine tries `renameat` first for each file and only enters the copy fallback after `EXDEV`; it uses IO-contract directory/file ops (`mkdirat`, `renameat`, `statx`, `openat`, `open_copy_file_session`, `copy_file_chunk`, `fchown`, `fchmod`, `fsync`, `close`, `unlinkat`, `close_copy_file_session`) rather than `opendir` / `readdir` / `fstatat` / raw copy loops. The public IO interface intentionally exposes the semantic copy-session API, not `splice` or `copy_file_range`; RealIO keeps `IORING_OP_SPLICE` private behind `copy_file_chunk`, and the non-io_uring readiness backends run `copy_file_chunk`/metadata ops on backend-owned file-op pools when the operation would otherwise block the event loop. The legacy `MoveJob.start()` / `startWithSpawner()` worker-thread entry points and whole-root mover tests were removed so future development has one relocation scheduling model. Non-io_uring daemon backends keep the same MoveJob call shape; their backend adapters may be thread-pool-backed internally for async file operations, but MoveJob does not branch into its own worker path based on backend choice. Basic ownership and permission/special-bit metadata is now fatal-on-error and preserved for copied files and directories created by MoveJob. Timestamps, xattrs, and ACLs are intentionally deferred pending a portability/fidelity decision. RealIO uses `IORING_OP_PIPE` when supported and `pipe2` as a fast setup fallback, and closes copy-session pipe fds with `IORING_OP_CLOSE`. A `copy_file_range` threadpool remains a possible future experiment only if profiling finds a concrete reason. Immediate follow-up: migrate the remaining older RealIO synchronous fallback paths (for operations such as bind/listen/setsockopt/truncate/openat-family fallback) onto the ready-completion queue so no fallback callback fires inline. Remaining broader follow-ups: `remove_delete_files` still needs an event-loop delete job, and peer `getpeername` / per-peer socket-option setup remain separate IO-contract cleanup candidates. See `progress-reports/2026-05-01-io-directory-ops.md`, `progress-reports/2026-05-02-io-directory-metadata.md`, `progress-reports/2026-05-03-move-job-event-loop.md`, `progress-reports/2026-05-05-io-contract-bypass-cleanup.md`, and `progress-reports/2026-05-06-movejob-splice-fallback.md`.
