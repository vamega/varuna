...
- **Switch RealIO.truncate to `IORING_OP_FTRUNCATE` once kernel floor bumps to 6.9+.** Currently RealIO.truncate is the only synchronous-completion op in the contract — it falls back to `posix.ftruncate(2)` because `IORING_OP_FTRUNCATE` (kernel 6.9, ~2024) is above varuna's 6.6 floor. When the floor rises to 6.9+, swap the body for `self.ring.ftruncate(...)` matching the existing fallocate/fsync shape; remove the synchronous-path comment from `src/io/real_io.zig`. The SimIO side and the contract are already async-shaped, so no caller changes. Estimated 30 minutes once the floor bump lands. Tracking AGENTS.md's broader floor-blocked op set: `IORING_OP_SETSOCKOPT` (6.7+), `IORING_OP_BIND/LISTEN` (6.11+).
