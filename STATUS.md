...
- **EventLoop integration scaffold (`tests/sim_smart_ban_eventloop_test.zig`)**: pre-drafted EventLoop(SimIO)-driven smart-ban test using option (1) bitfield layout — corrupt peer = `{0}`, honest peers = `{1, 2, 3}` — so the daemon's rarest-first picker forces piece 0 onto the corrupt peer deterministically. Asserts the option-(1) shape so it doesn't drift before the swap.
