...
- **Async SimHasher model — interleaving realism for hash-result delivery.** Today `SimHasher.submitVerifyEx` (`src/io/hasher.zig:657`) computes the hash and pushes the result onto `completed_results` synchronously — effectively zero latency. Real `RealHasher` delivers results from a worker pool with delays of ~5–15 ms per piece; during that window the EL is processing peer messages and advancing handshakes, and *that* interleaving is where historical bugs lived. Proposal: parameterize delivery delay in ticks via a `hash_jitter_max_ticks` knob (default 0 = preserves current behavior), seeded through the existing `*Random`, so BUGGIFY × 32 seeds explores "hash arrives mid-handshake" / "hash arrives after slot freed" / "hash arrives after peer disconnect" interleavings. Determinism stays tight as long as the jitter sequence flows through `*Random`. Estimated 0.5–1 day. Direct successor to the SimHasher determinism work and the MSE handshake harness (which couldn't fully prove the simultaneous-handshake fix because the strict model can't reproduce the original interleaving).
- **SimIO fault knobs — `cqe_reorder_window_ticks` + `delayed_close_cqe_max_ticks`.** Today's SimIO closes synchronously: `closeSocket(fd)` immediately fails the parked recv with `ConnectionResetByPeer`. Real io_uring enqueues close as another op; the parked recv's CQE arrives *eventually*, sometimes after many other CQEs have fired. The interleaving "fd closed → slot reused → new connect submitted → old recv CQE finally arrives, lands on slot whose state has changed" is the entire family of bugs the MSE harness was hunting and couldn't reproduce. Proposal: a fault knob on `SimIO.FaultConfig` that lets close-fired CQEs land at a configurable later tick, plus a `cqe_reorder_window_ticks` knob that allows arbitrary same-tick CQEs to be permuted (within a bounded window). Combined with `*Random` seeding, this gives reproducible exploration of the late-CQE space. Touches `src/io/sim_io.zig` close paths + the heap fire ordering. Estimated 1–2 days. Strongest evidence-of-need: the MSE round found 2 real `posix.close` bugs but couldn't *prove* the original simultaneous-handshake race is gone on real hardware — the strict SimIO model is structurally incapable of reproducing it. See `progress-reports/2026-04-30-mse-handshake-race.md` for the framing.
- **EL-driven MoveJob via IO-contract directory ops** (also unblocks EL-driven `PieceStore.init`). Today MoveJob spawns a fresh `std.Thread.spawn` per job and walks directories via raw `posix.openat` / `opendir` / `readdir` / `mkdirat` / `unlinkat` / `rename` / `copy_file_range` calls — outside the deterministic envelope, no simulation possible. PieceStore.init has the same shape on its synchronous mkdir+open phase. Both can become EL-driven once we add the missing directory ops to the IO contract: `IORING_OP_GETDENTS` (kernel ≥6.7, see `liburing` `io_uring_prep_getdents`), `IORING_OP_OPENAT` / `OPENAT2` (kernel ≥5.6 / 5.6), `IORING_OP_MKDIRAT` (kernel ≥5.15), `IORING_OP_UNLINKAT` (kernel ≥5.11), `IORING_OP_RENAMEAT` (kernel ≥5.11). Each needs the standard tagged-union shape: `RealIO` async via SQE when `feature_support.supports_*` is set + sync `posix.*` fallback below the floor; `SimIO` synthetic-fd implementations with fault knobs (parented at virtual filesystem state); the four POSIX dev backends ship sync impls. Once these land, MoveJob can be rewritten as an EL-thread state machine using `splice` / `copy_file_range` (already on the contract). **Concurrency design open question:** today's "1 thread per job" trivially serialises within a job but unbounded across jobs. A future EL-driven MoveJob needs a queueing / scheduling policy: cap simultaneous job count (single-disk contention argues for 1–2), partition by destination filesystem dev_t (multiple disks can move in parallel), prioritise by job-size or by user request order. File alongside if/when we implement this — the policy is non-obvious and depends on operator workload. Estimated 1–2 weeks for the full directory-ops contract surface; MoveJob v2 rewrite is small once that's in place. Side benefit: same primitives let us simulate filesystem fault injection (ENOSPC, EXDEV, EIO) deterministically across 32 seeds.
...
- **`runtime.probe.ensureSupported` blocks daemon startup on missing io_uring even under `-Dio=epoll_*`.** `src/runtime/probe.zig:23-26` unconditionally returns `error.IoUringUnavailable` when `summary.io_uring_available == false`, regardless of which backend the binary was compiled for. A daemon built with `-Dio=epoll_posix` or `-Dio=epoll_mmap` (the whole point of which is to run on systems without io_uring) currently refuses to boot on such systems with `startup blocked: io_uring is unavailable on this host` — defeating the readiness backends entirely on their target environment. Fix: gate the io_uring check on `build_options.io_backend == .io_uring` (e.g. import the build option in `probe.zig` and only fail on missing io_uring when the selected backend actually needs it). The kernel-floor minimum (currently 6.6, set in `src/runtime/requirements.zig`) probably also wants a per-backend override — epoll requires only kernel 2.6, so requiring 6.6 is a wasted gate when the daemon was built without io_uring. Estimated 2–4 hours; the surface is small. Surfaced by 2026-04-30 backend-validation. See `progress-reports/2026-04-30-backend-validation.md`.
- **`scripts/demo_swarm.sh` baseline regression — pieces no longer transfer under `-Dio=io_uring`.** Both daemons connect, complete MSE + BEP-10 extension handshakes (peer extensions are logged on both sides), but no BITFIELD / INTERESTED / UNCHOKE / PIECE messages follow and `progress` stays at 0.0 until the 60 s test timeout. Reproduces with `encryption=disabled` and `enable_utp=false` (TCP-only, plaintext) — connection storm of 4 peer slots per side suggests both daemons dial AND accept simultaneously. The in-process `transfer_integration_test` still passes, so the wire-protocol logic in `src/io/peer_handler.zig` / `src/io/protocol.zig` is intact at the algorithm layer; the failure is at the daemon-glue boundary across two real processes. Last verified passing at commit `3ab5f59` (2026-04-30 MSE merge) per `progress-reports/2026-04-30-mse-handshake-race.md` ("Verified with 5 consecutive demo_swarm runs"); fails as of `8633644` (current main). Suspects to investigate: the connection-storm reaping (which slots close, which keep the BT chain alive); whether seeder's `entering seed mode` triggers a fresh announce as `event=completed` and whether that re-builds peer state; whether `peer_policy.tryAssignPieces` ever sees a peer with `availability != null` (i.e. did BITFIELD arrive). Blocks Phase 2 of cross-backend validation — epoll backends inherit the same baseline failure. Estimated 0.5–2 days depending on which surface is broken. See `progress-reports/2026-04-30-backend-validation.md`.
- **Cross-backend perf comparison vs `io_uring` not measured.** Phase 3 of backend validation (relative throughput / latency under `-Dio=epoll_posix` vs `-Dio=epoll_mmap` vs `-Dio=io_uring`) was blocked on the `demo_swarm.sh` baseline regression above. The CPU microbenchmarks in `src/bench/main.zig` don't exercise the IO backend (kernel parser / bencode / SHA-1 / metainfo parse only), so they produce identical numbers across backends. A meaningful comparison needs either (a) a working end-to-end swarm transfer, or (b) a focused EL-driven micro-benchmark that times N timeout cycles + N socket recv/send round-trips through the contract surface. Filing as a follow-up; once the swarm regression closes, the perf delta is one demo_swarm run per backend with `time(1)` wrapping the transfer phase. Estimated 0.5 day to land both bench harness and numbers. See `progress-reports/2026-04-30-backend-validation.md`.
