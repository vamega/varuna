...
- HTTP, HTTPS, and UDP tracker announce (BEP 15) with compact peer lists, multi-tracker simultaneous announce (BEP 12). All tiers queried in parallel; first successful response wins. Async DNS resolution with TTL-based caching (`src/io/dns.zig`). Build-time configurable backend: threadpool (default), c-ares (`-Ddns=c-ares`), or custom (`-Ddns=custom`; HTTP/UDP executors instantiate the IO-backed resolver directly). HTTPS via vendored BoringSSL with BIO pair transport (all network I/O stays on io_uring); build-time configurable: `-Dtls=boringssl` (default) or `-Dtls=none`.
- Full UDP tracker support (BEP 15): connect/announce/scrape protocol, connection ID caching (2-minute TTL), exponential backoff retries (15 * 2^n seconds), error response handling. io_uring-based async executor (`IORING_OP_SENDMSG`/`IORING_OP_RECVMSG`) owns all live UDP tracker I/O; `src/tracker/udp.zig` is now packet codec/cache logic only. Daemon auto-detects `udp://` URLs and routes to UDP executor.
...
- BEP 11 Peer Exchange (PEX): parse incoming ut_pex messages (added/dropped IPv4/IPv6 peers with flags), build and send outbound PEX messages every ~60s with delta encoding, enqueue PEX-discovered peers through the in-memory peer candidate list, private torrent enforcement (PEX completely disabled).
...
- BEP 9 Magnet Link Resilience: event-loop `AsyncMetadataFetch` handles multi-peer retry, peer deduplication, failed-peer backoff, and piece-by-piece SHA-1 verification without the old blocking socket client. Metadata fetch progress reporting lives in `src/net/metadata_progress.zig` and is exposed through `Stats` and API.
...
- **Fail-closed startup/config policy**: malformed config files now abort startup instead of silently falling back to defaults; invalid encryption modes are rejected; explicit `io_uring` daemon startup blocks on unsupported kernels or missing `io_uring`; removed dead config knobs `connect_timeout_secs`, `performance.pipeline_depth`, and `performance.ring_entries`.
- **Runtime-selectable daemon IO backend**: one Linux daemon binary can select `io_uring`, `epoll_posix`, or `epoll_mmap` at startup through `[daemon] io_backend`. The default `auto` mode prefers `io_uring` when the runtime probe succeeds and falls back to `epoll_posix` when `io_uring` is unavailable; explicit `io_uring` remains fail-closed. macOS `kqueue_posix` / `kqueue_mmap` branches are wired for macOS selection and still need real Darwin validation.
- **Backend swarm validation**: `zig build test-swarm-backends` runs the live tracker/seeder/downloader demo across `io_uring`, `epoll_posix`, and `epoll_mmap` with runtime config pinned per daemon. The 2026-05-01 matrix passes for all three Linux backends with 1 MiB payloads; macOS kqueue validation remains a separate Darwin-host follow-up.
- **Backend swarm perf harness**: `zig build perf-swarm-backends` runs the same live tracker/seeder/downloader matrix in perf mode. It records run number, backend, status, elapsed seconds, transfer seconds, payload bytes, throughput MiB/s, work directory, and run log in `perf/output/backend-swarm-perf-*/summary.tsv`. Defaults are local/manual friendly (`PAYLOAD_BYTES=16777216`, `RUNS=1`, Linux backends), with `BACKENDS`, `PAYLOAD_BYTES`, `RUNS`, `TIMEOUT`, `PORT_BASE`, and `OUT_DIR` overridable from the environment.
...
- **Resume/state integrity hardening**: `ResumeWriter.flush()` now swaps and re-queues batches instead of clearing a shared pending slice after unlock, `clearTorrent()` now removes all torrent-scoped resume tables in one transaction, live piece completions are queued to SQLite only after their writes cross a successful per-torrent fsync/fdatasync barrier, and v2 multi-piece piece checks now fail closed with `DeferredMerkleVerificationRequired` instead of accepting arbitrary payloads.
...
- **Queue/runtime operational cleanup**: queue enforcement consolidated into `QueueManager.enforce()` (single entry point for both promotion and demotion), runtime DHT toggles flip `engine.enabled` instead of dropping the engine pointer, the legacy `setLocation()` synchronous file-move path has been removed in favor of the async `POST /api/v2/varuna/torrents/move` endpoint (closes review C5), and `/sync/maindata` now returns the sync body directly instead of arena-building then duplicating it.
...
- **Peer listener accept carries peer address**: the shared `EventLoop` listener passes caller-owned address storage to `accept`; RealIO uses single-shot accept plus callback rearm for this path so inbound peers get a real remote address without a follow-up `getpeername` syscall.
...
- **Core**: webapiVersion, version, buildInfo, preferences (40+ fields), setPreferences (form + JSON), transfer/info (with connection_status, real DHT node count from routing table), speedLimitsMode, defaultSavePath, toggleSpeedLimitsMode (intentionally unsupported), transfer/downloadLimit, transfer/uploadLimit, transfer/setDownloadLimit, transfer/setUploadLimit.
- **Torrents**: info (40+ fields matching qui Torrent interface, real infohash_v2 for BEP 52 torrents, distributed availability, popularity), add (multipart + raw), delete, pause, resume, properties (30+ fields with hash, name, created_by, creation_date, scrape-based peers_total/seeds_total, v2 info-hash, completion_date, wasted bytes, average speeds), files (with index, availability, real piece_range), trackers (with msg field), pieceStates, pieceHashes, export (.torrent file).
- **Controls**: filePrio, setSequentialDownload, toggleSequentialDownload, setDownloadLimit, setUploadLimit, downloadLimit, uploadLimit, forceReannounce, recheck, setLocation, connDiagnostics, setShareLimits, rename, setForceStart, setAutoManagement (stub), renameFile (intentionally unsupported), renameFolder (intentionally unsupported), addPeers.
- **Categories & Tags**: categories (create/edit/remove/list/setCategory), tags (create/delete/addTags/removeTags/list).
- **Sync**: /api/v2/sync/maindata delta protocol (rid-based, Wyhash change detection, 100-snapshot circular buffer, real DHT node count and total peer connections in server_state, temporary 100 GiB free-space placeholder, infohash_v2 in torrent objects), sync/torrentPeers with real peer data (IP, flags, progress, transfer stats, per-peer dl/ul speed, client name from peer ID).
...
- Torrent data relocation: async file-move job + new `POST /api/v2/varuna/torrents/move` (returns job id), `GET /move/<id>` for progress polling, `/cancel`, `/commit`, `DELETE` for cleanup. Production relocation is manifest-scoped and event-loop driven through the IO contract; same-fs moves use `renameat`, cross-fs moves use `openat` + `copy_file_chunk` + destination `fchown`/`fchmod` + `fsync` + `unlinkat`. RealIO implements the semantic copy session with a backend-owned pipe and private `IORING_OP_SPLICE` state machine; `IORING_OP_PIPE` is used when available and otherwise falls back to `pipe2(O_CLOEXEC|O_NONBLOCK)` with next-tick ready completion. POSIX and mmap readiness backends offload explicit file, metadata, copy, socket setup, fd teardown, and mmap mapping setup syscalls to backend-owned `BlockingOpPool` workers; mmap read/write still carry the documented page-fault limitation. Atomic progress counters; `O_NOFOLLOW` symlink safety. Basic ownership and permission/special-bit metadata is preserved for copied files and created directories; timestamps, xattrs, and ACLs remain a feasibility/fidelity follow-up. The qBittorrent-compatible `setLocation` endpoint now returns 400 with a body that names the new endpoint (its synchronous semantics are incompatible with the daemon's performance guarantees). `copy_file_range` on a backend-owned threadpool remains a possible future experiment only if profiling finds a concrete reason. See `progress-reports/2026-04-30-async-move-job.md`, `progress-reports/2026-05-03-move-job-event-loop.md`, `progress-reports/2026-05-05-io-contract-bypass-cleanup.md`, `progress-reports/2026-05-06-movejob-splice-fallback.md`, `progress-reports/2026-05-06-blocking-op-pool-syscalls.md`, and `progress-reports/2026-05-06-close-and-mmap-setup-offload.md`.
- Per-torrent connection diagnostics: connection attempts/failures/timeouts exposed via connDiagnostics API.
- Partial download cleanup: delete torrent with --delete-files now enqueues a manifest-scoped event-loop DeleteJob that removes data files and prunes empty directories through the IO contract (`unlinkat`, `openat`, `getdents`, `close`).
...
- Event loop integration (`src/io/dht_handler.zig`): DHT/uTP demux by first byte ('d' for KRPC, else uTP). DHT tick in event loop. Outbound packets sent via shared UDP socket. Discovered peers feed the in-memory peer candidate list so connection-limit pressure does not discard DHT results.
...
- SO_BINDTODEVICE tests (`zig build test-bind-device`): `applyBindDevice` validation for the c-ares/startup-probe helper in `src/net/socket.zig`; daemon peer sockets use the IO contract for setup.
- strace policy validator (`zig build validate-strace -- <summary>`): automated verification that daemon network I/O routes through `io_uring_enter` with no direct `connect`/`send`/`recv`/`sendto`/`recvfrom`/`sendmsg`/`recvmsg` syscalls.
...
- `pub fn EventLoopOf(comptime IO: type) type` in `src/io/event_loop.zig` is the daemon event loop, generic over its IO backend. The daemon startup path now instantiates the concrete `EventLoopOf(IO)` selected at runtime (`io_uring`, `epoll_posix`, or `epoll_mmap` on Linux). `EventLoop = EventLoopOf(RealIO)` remains the build-selected alias for tests/tools, and `EventLoopOf(SimIO)` is the deterministic-simulation type used by sim tests.
...
- **`RealIO` (`src/io/real_io.zig`)**: io_uring backend implementing the same interface. Encodes `Completion*` as SQE `user_data`; multishot accept honours `IORING_CQE_F_MORE`, while caller-address accept uses single-shot SQEs plus callback rearm; connect deadlines submit a paired `link_timeout`. `dispatchCqe` clears `in_flight` *before* invoking the callback (multishot still preserves the flag) so callbacks can re-arm the same completion in the natural "header → body → next header" pattern.
...
- **Stage 2 (event-loop migration) — DONE.** Every async op in the daemon runs through the io_interface backend. The legacy `ring: linux.IoUring` field, the giant CQE dispatch switch, `OpType` / `OpData` / `encodeUserData` / `decodeUserData` are all deleted.
  ...
  - Peer recv/send, outbound peer socket/setup/connect, peer accept with remote address, signal poll, recheck reads, disk reads/writes (including PieceStore.sync's async fsync), HTTP executor (socket/connect/send/recv with deadline-bounded connect), RPC server (accept/recv/sendmsg with per-op gen-stamped tracking struct), metadata fetch (socket setup/connect/send/recv per Slot), uTP recvmsg/sendmsg, UDP tracker (socket/sendmsg/recvmsg per Slot), timerfd → native io.timeout — all migrated.
...
- ~~**uTP BT send bridge**~~: (DONE) uTP data transfer works end-to-end. Fixed seq_nr off-by-one in `acceptSyn` and inbound state machine stall after extension handshake. `zig build test-swarm` runs with TCP+uTP.
...
- **Seed immediately after torrent completion**: when the last wanted piece is verified and persisted, transition to the seeding state and announce as a seed immediately instead of running a full completion recheck first. If we still want end-of-download belt-and-suspenders validation, run it as a background verification pass after seeding has begun; any piece that fails the pass should be demoted and re-downloaded rather than blocking initial seed availability.
...
- ~~**Broader RPC arena coverage**~~: (DONE 2026-05-02) production RPC handlers run under the per-slot `TieredArena`, and the remaining dirty-cache hole in category/tag list responses now builds directly into the caller/request allocator instead of first populating parent-owned cached JSON. Regression coverage in `tests/rpc_arena_test.zig` asserts dirty category/tag caches do not change parent allocator live allocation counts across a request-arena-backed handler call. `SyncState` snapshot hash maps remain persistent delta state, not per-request transient work.
...
- ~~**Dynamic outbound buffer for UtpSocket**~~: (DONE 2026-05-04) `UtpSocket.out_buf` is now bounded dynamic storage (`max_outbuf = 512`) with a head index and occasional compaction, so high-throughput uTP can keep a larger retransmission flight without reserving ~177 KiB per idle socket or copying the buffer on every cumulative ACK. See `progress-reports/2026-05-04-utp-dynamic-retransmit-buffer.md`.
...
- ~~**Live-pipeline BUGGIFY harness for AsyncRecheckOf(SimIO)**~~: Closed 2026-05-02. `AsyncRecheck` is generic over its IO backend (`AsyncRecheckOf(IO)` in `src/io/recheck.zig`; daemon callers stay on the `AsyncRecheckOf(RealIO)` alias), and `tests/recheck_live_buggify_test.zig` now runs the canonical SimIO live-pipeline BUGGIFY wrapper: 32 seeds across happy-path, corrupt-piece, and known-complete fast-path scenarios with per-tick random faults and op fault knobs. Verification on 2026-05-02 passed `zig build test-recheck-live-buggify`: happy-path and fast-path seeds completed, corrupt-piece seeds all caught the bad piece, and the summaries retained the expected full/partial verification split under injected faults. See `progress-reports/2026-05-02-async-recheck-live-buggify.md`.
...
- ~~**Happy-path `AsyncMetadataFetchOf(SimIO)` integration test.**~~ Closed 2026-04-29; socket setup cleanup landed 2026-05-07. `connectPeer` now submits via `self.io.socket()` instead of the synchronous `posix.socket()` (new `SlotState.socket_creating` + `metadataSocketComplete` callback), routes peer socket tuning through `self.io.setsockopt()`, and then chains `connect`; `releaseSlot` routes through `self.io.closeSocket()`. Two SimIO additions wire the test: `enqueueSocketResult(fd)` (next `socket()` returns this specific fd) and `pushSocketRecvBytes(fd, bytes)` (the scripted-peer mirror of `setFileBytes`, including the parked-recv wake-up path). Happy-path test (`tests/metadata_fetch_test.zig`): one peer, scripted BT handshake reply + extension handshake (with `metadata_size` and a non-zero `ut_metadata` ID) + ut_metadata data response carrying a 256-byte info dictionary. Assertions: `completed`, `had_metadata`, `result_len == 256`, first/last byte match. See `progress-reports/2026-04-29-executor-genericization.md`.
...
- ~~**Daemon submission paths for bind / listen / setsockopt.**~~ Listener bring-up landed 2026-04-30; helper cleanup landed 2026-05-05; blocking-syscall fallback cleanup landed 2026-05-06; per-peer socket setup cleanup landed 2026-05-07. Contract ops `bind` / `listen` / `setsockopt` in `src/io/io_interface.zig` (`BindOp` / `ListenOp` / `SetsockoptOp`) remain the only IO-interface surface. `RealIO` impls branch on `feature_support.supports_bind` / `supports_listen` / `supports_setsockopt` — async via `IORING_OP_BIND` / `IORING_OP_LISTEN` / `IORING_OP_URING_CMD+SOCKET_URING_OP_SETSOCKOPT` when supported, backend-owned `BlockingOpPool` fallback otherwise. SimIO remains deterministic; EpollPosixIO / EpollMmapIO / KqueuePosixIO / KqueueMmapIO route these syscall-shaped ops through their backend-owned pool rather than running them inline on the event-loop thread. The former `io_interface.*Blocking` wrappers were removed because the IO contract should not advertise caller-blocking conveniences; one-shot listener startup now owns its local "submit + tick until done" wait loops in `event_loop.zig` and `rpc/server.zig`. Peer TCP_NODELAY / socket buffer tuning, outbound peer SO_BINDTODEVICE, and outbound bind-address setup now run as ordered IO-contract setup before connect or inbound handshake receive. `main.zig` startup port-scan stays synchronous (pre-event-loop, no IO instance available). See `progress-reports/2026-04-30-bind-listen-setsockopt.md`, `progress-reports/2026-05-05-io-contract-bypass-cleanup.md`, `progress-reports/2026-05-06-blocking-op-pool-syscalls.md`, and `progress-reports/2026-05-07-peer-socket-setup-io-contract.md`.
- ~~**Live-pipeline BUGGIFY harness for `PieceStoreOf(SimIO)`.**~~ Closed 2026-04-30. `tests/storage_writer_live_buggify_test.zig` runs the canonical 32-seed BUGGIFY pattern across 4 scenarios mirroring the foundation tests: init+sync (multifile errdefer-cleanup surface), fallocate→truncate fallback, 2-span writePiece/readPiece round-trip, 3-span writePiece/readPiece round-trip. Per-op `FaultConfig` at 5% per knob (10% on `fallocate_unsupported` to drive the truncate fallback) plus per-tick `injectRandomFault` at p=0.05 wired through a new `SimIO.pre_tick_hook` (callback + ctx pair fired at the top of every `tick`) — necessary because PieceStore's drain loops live inside its methods (`while (pending > 0) try io.tick(1)`), so no external tick boundary exists for the test to wrap. Run: 32/32 seeds terminate gracefully across all 4 scenarios; vacuous-pass guards pass with seeds-with-hits 3/3/2/3 of 32; successful runs 20/27/21/20 with the rest returning kernel-shaped errors (NoSpaceLeft, InputOutput, OperationNotSupported). No bugs surfaced. Wired through `zig build test-storage-writer-live-buggify`. See `progress-reports/2026-04-30-piece-store-live-buggify.md`.
...
- ~~**Async SimHasher model — interleaving realism for hash-result delivery.**~~ Closed 2026-05-01. `SimHasher.FaultConfig` now has `verify_result_delay_ticks_min` / `verify_result_delay_ticks_max` knobs, backed by a delayed verification-result queue that promotes results on `drainResultsInto` ticks. Defaults preserve the existing immediate-drain behavior, while tests can force deterministic "hash completes after other simulated work" interleavings. `hasPendingWork` and `deinit` both account for delayed results, so late-completion tests retain the same ownership and leak-checking semantics as immediate completions. See `progress-reports/2026-05-01-simhasher-jitter.md`.
- ~~**SimIO fault knobs — `cqe_reorder_window_ticks` + `delayed_close_cqe_max_ticks`.**~~ Closed 2026-05-01. `SimIO.FaultConfig` now exposes min/max tick ranges for delayed close-triggered reset CQEs and global CQE reordering. Pending completions carry a logical `ready_tick` in addition to their simulated nanosecond deadline, and `tick` scans for due+ready completions so a delayed earlier CQE does not block later ready CQEs. Defaults keep the strict model; opt-in tests can now force parked-recv resets and otherwise-ready completions to arrive on later event-loop ticks. See `progress-reports/2026-05-01-simio-late-cqes.md`.
- ~~**EL-driven MoveJob via IO-contract directory ops**~~ Closed 2026-05-03; thread-path cleanup landed 2026-05-05; copy-session cleanup landed 2026-05-06; RealIO ready-fallback and blocking-op-pool fallback cleanup landed 2026-05-06; delete-files cleanup landed 2026-05-07; peer accept/socket setup cleanup landed 2026-05-07. Production relocation is manifest-scoped and event-loop driven: `SessionManager.startMoveJob` constructs `MoveJob.createForFiles`, calls `MoveJob.startOnEventLoop`, and `SessionManager.tickMoveJobs` advances each active job on the shared event loop. The MoveJob state machine tries `renameat` first for each file and only enters the copy fallback after `EXDEV`; it uses IO-contract directory/file ops (`mkdirat`, `renameat`, `statx`, `openat`, `open_copy_file_session`, `copy_file_chunk`, `fchown`, `fchmod`, `fsync`, `close`, `unlinkat`, `close_copy_file_session`) rather than `opendir` / `readdir` / `fstatat` / raw copy loops. The public IO interface intentionally exposes the semantic copy-session API, not `splice` or `copy_file_range`; RealIO keeps `IORING_OP_SPLICE` private behind `copy_file_chunk`, and all non-native syscall fallbacks now route through backend-owned `BlockingOpPool` plumbing instead of blocking the event-loop thread. The legacy `MoveJob.start()` / `startWithSpawner()` worker-thread entry points and whole-root mover tests were removed so future development has one relocation scheduling model. Non-io_uring daemon backends keep the same MoveJob call shape; their backend adapters may be thread-pool-backed internally for async file operations, but MoveJob does not branch into its own worker path based on backend choice. Basic ownership and permission/special-bit metadata is now fatal-on-error and preserved for copied files and directories created by MoveJob. Timestamps, xattrs, and ACLs are intentionally deferred pending a portability/fidelity decision. RealIO uses `IORING_OP_PIPE` when supported and `pipe2` as a fast setup fallback, and closes copy-session pipe fds with `IORING_OP_CLOSE`. Older RealIO unsupported-ring fallbacks now run on the blocking-op pool and wake `tick` through eventfd, so `close`, `truncate`, `openat`/directory ops, `statx`, `getdents`, `socket`, `bind`, `listen`, `setsockopt`, `fchown`, and `fchmod` do not run on the event-loop submission path when the ring op is unavailable. Mmap backend mapping setup and fd teardown now also route through the blocking-op pool; mmap read/write still carry the documented page-fault limitation. A `copy_file_range` threadpool remains a possible future experiment only if profiling finds a concrete reason. `remove_delete_files` now uses a manifest-scoped event-loop DeleteJob over IO-contract `unlinkat` / `openat` / `getdents` / `close`; peer inbound accept now carries the remote address and peer socket setup uses ordered IO-contract ops. See `progress-reports/2026-05-01-io-directory-ops.md`, `progress-reports/2026-05-02-io-directory-metadata.md`, `progress-reports/2026-05-03-move-job-event-loop.md`, `progress-reports/2026-05-05-io-contract-bypass-cleanup.md`, `progress-reports/2026-05-06-movejob-splice-fallback.md`, `progress-reports/2026-05-06-realio-ready-fallbacks.md`, `progress-reports/2026-05-06-blocking-op-pool-syscalls.md`, `progress-reports/2026-05-06-close-and-mmap-setup-offload.md`, `progress-reports/2026-05-07-delete-files-event-loop.md`, and `progress-reports/2026-05-07-peer-socket-setup-io-contract.md`.
- ~~**Remove IO-interface blocking helper surface.**~~ Closed 2026-05-05. `io_interface.{socket,bind,listen,setsockopt,bindDevice}Blocking` are gone; the contract exposes core async operations only. Startup callers that need a synchronous bring-up boundary own their local wait loop around the async op, which keeps that policy at the call site instead of advertising it as acceptable general IO-interface usage.
- ~~**`runtime.probe.ensureSupported` blocks daemon startup on missing io_uring even under `-Dio=epoll_*`.**~~ Closed 2026-05-01. `runtime.probe.selectRuntimeIoBackend` now chooses the effective backend from `[daemon] io_backend`, and only the explicit/effective `io_uring` path enforces the kernel floor plus `io_uring` availability. Linux `auto` falls back to `epoll_posix` when the probe fails; explicit `epoll_posix` / `epoll_mmap` bypass the io_uring gate. See `progress-reports/2026-05-01-runtime-io-backend-selection.md`.
- ~~**`varuna-automation swarm` baseline regression — pieces no longer transfer under `-Dio=io_uring`.**~~ Closed 2026-05-01. The regression no longer reproduces on current `main`: two clean local-swarm runs under the default io_uring backend completed with `progress=1.0000` and verified the transferred payload with `cmp`. The earlier STATUS entry appears to have been made stale by subsequent peer-protocol / smart-ban / test-stability fixes rather than a dedicated demo-swarm patch. Keep `zig build test-swarm` as the real two-daemon smoke gate before cross-backend perf work. See `progress-reports/2026-05-01-demo-swarm-baseline.md`.
- ~~**Cross-backend perf comparison vs `io_uring` not measured.**~~ Closed 2026-05-01 for the practical live-swarm harness. `zig build perf-swarm-backends` runs `varuna-automation backend-swarm` in perf mode, records per-backend transfer timing and throughput, and preserves each run's logs under `perf/output/backend-swarm-perf-*`. Treat the numbers as loopback live-swarm comparisons, not isolated event-loop microbenchmarks; use larger `PAYLOAD_BYTES` and `RUNS>1` when comparing small backend deltas. See `progress-reports/2026-05-01-backend-swarm-perf.md`.
...
- ~~**MSE handshake failures in mixed encryption mode**~~: Closed 2026-04-30. Same surface as the "MSE simultaneous handshake robustness" entry above; the symptoms (`vc_not_found` / `req1_not_found` from cross-handshake state corruption) do not reproduce across 32 deterministic seeds + recv-error fault injection under SimIO + SimHasher + SimRandom. The old local-swarm disabled-encryption note was already stale: the local swarm harness uses `encryption = "preferred"` as of commit `3284849`. See `progress-reports/2026-04-30-mse-handshake-race.md`.
...
- `IORING_OP_SETSOCKOPT` (kernel 6.7+), `IORING_OP_BIND`/`LISTEN` (kernel 6.11+) not available on current kernel 6.6, so RealIO routes these ops through the backend-owned blocking-op pool when unsupported by the ring. Peer socket tuning and outbound bind-device/bind-address setup now use those IO-contract paths.
...
updated to reflect the fix — note that the local swarm harness
...
- `src/tracker/udp.zig` — `generateTransactionId(random)`. The production
...
- `6150f85` — `io: add truncate op to contract; sync RealIO + SimIO with fault knob`. New `TruncateOp` + `Result.truncate` variant on `io_interface.zig`. At that point RealIO used a synchronous `posix.ftruncate` fallback because `IORING_OP_FTRUNCATE` requires kernel 6.9 (above varuna's 6.6/6.8 floor); later work added runtime `IORING_OP_FTRUNCATE` probing and moved the fallback syscall onto the backend-owned `BlockingOpPool`. SimIO implementation plus `truncate_error_probability` knob (delivers `error.InputOutput`); companion entries added to `cancelResultFor` + `buggifyResultFor`. Three algorithm tests in `tests/sim_socketpair_test.zig` (success, fault, plus an inline RealIO test asserting the file actually grew on disk).
...
Since the 2026-05-07 IO-contract cleanup, metadata-fetch sockets
route TCP_NODELAY / SNDBUF / RCVBUF tuning through `self.io.setsockopt`
before connect. The old blocking `configurePeerSocket` helper has been
removed; real and simulated backends now share the same setup contract.
...
### Follow-ups filed
- **Live-pipeline BUGGIFY harness for `AsyncRecheckOf(SimIO)`** — closed
  2026-05-02; see the operational subsection above and
  `progress-reports/2026-05-02-async-recheck-live-buggify.md`.
...
- **Three new test files** matching the layered testing strategy:
  ...
  - `tests/sim_smart_ban_phase12_eventloop_test.zig` — Phase 1-2 integration test, with harness smoke + disconnect-rejoin discriminating-power live. Two scenarios (two-peer corruption, steady-state honest co-located) remain deferred until the harness can force deterministic same-piece co-contribution.
...
- **RPC arena ownership follow-up (2026-05-01)**: `TieredArena.ownsSlice()` now checks non-empty response slices against both the slab backing and live spill allocations. `ApiServer` uses that precise check for `owned_body` and `owned_extra_headers`, so parent-allocated owned response memory is still freed through the server allocator while arena-backed response memory is left for `arena.reset()`. Regression coverage returns a parent-owned body and extra headers while a per-slot arena exists and asserts the parent allocator returns to its pre-request baseline.
...
- **Test coverage**: original Stage 2 added 7 algorithm + integration + safety-under-fault tests in `tests/rpc_arena_test.zig`; the 2026-05-01 follow-up adds a parent-owned response integration regression plus a `TieredArena.ownsSlice()` slab/spill inline test. No leaks across the RPC arena suite under the GPA leak detector.
...
- **Test coverage**: the three wanted-completed-count cache regression guards now live inline in `src/torrent/piece_tracker.zig` (complete-with-mask, mask-transition recompute, mask-removal recompute). The BUGGIFY cache stress test remains in `tests/piece_tracker_cache_buggify_test.zig`.
...
- Demo swarm end-to-end with uTP enabled: `zig build test-swarm` runs TCP+uTP, seeder-to-downloader transfer verified
