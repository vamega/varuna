...
- **Multi-source piece assembly + smart-ban Phase 1-2 (next-piece, scheduled)**: each piece is currently downloaded from a single peer; requesting different blocks from different peers improves download performance and is a prerequisite for full smart-ban value. Smart-ban Phase 1 (per-block SHA-1 attribution) and Phase 2 (cross-peer corruption identification in multi-source assembly) build on top. The work follows the project's simulation-first testing philosophy: sim tests are written first against `EventLoopOf(SimIO)` + `SimPeer` to lock in the assembly + attribution semantics, before the production code paths land. Sim-engineer writes the test scaffolds; migration-engineer implements production in parallel once the API surface is stable.
...
- **STYLE.md migration-pattern catalogue** at 13 entries: patterns 1-7 (Stage 2 lessons), 8-10 (single-coherent-commits, bench-companion, lazy-compilation-shipping), 11 (Stage 5 closeout); the new "Layered Testing Strategy" top-level section codifies the algorithm/integration/safety-under-faults split. Patterns 12 (`closeSocket` on the IO interface) and 13 (`max_ops_per_tick` modelling kernel CQE batches) are migration-engineer's follow-up.
