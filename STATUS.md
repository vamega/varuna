...
- ~~**Live-pipeline BUGGIFY harness for `PieceStoreOf(SimIO)`.**~~ (DONE 2026-04-29) `tests/storage_writer_buggify_test.zig` — 32-seed harness across 4 recovery paths: (1) 5-file partial-init cleanup under `fallocate_error_probability = 0.3` (26/32 seeds hit fault), (2) fsync error storm at p ∈ [0.5, 1.0] (32/32 hit), (3) 3-span write/read mixed faults at p ∈ [0.1, 0.7] (26/32 hit, 40 total hits across write+read phases), (4) writePiece/readPiece short-write/short-read loops at p=0.7 (32/32 round-trip success). Two new `FaultConfig` knobs (`short_read_probability` / `short_write_probability`) added to SimIO so the per-span resubmit loops can be exercised. Vacuous-pass guards on every test. No production bug surfaced — confidence-builder outcome on top of the 2026-04-28 storage IO-contract and truncate-op landings. See `progress-reports/2026-04-29-piece-store-buggify-harness.md`.
...
## Last Verified Milestone (2026-04-29 — PieceStoreOf(SimIO) live-pipeline BUGGIFY harness)

Closes the open follow-up at `STATUS.md` (line 293, now closed). New `tests/storage_writer_buggify_test.zig` wraps the foundation `PieceStoreOf(SimIO)` integration tests with the canonical 32-seed BUGGIFY shape over 4 recovery paths. Two new `FaultConfig` knobs (`short_read_probability`, `short_write_probability`) added to `src/io/sim_io.zig` so the per-span resubmit loops in `writePiece` / `readPiece` get exercised — independent of the corresponding `_error_probability` knobs, success-path only, strict short returns in `[1, op.buf.len)`. Defaults are 0.0 so existing callers are unaffected.

**What landed**

- `src/io/sim_io.zig` — new `FaultConfig.short_read_probability` and `FaultConfig.short_write_probability` fields. `read` honours short_read on the registered-content path (legacy "no content registered → 0 bytes" path unchanged). `write` honours short_write on the success path. Both knobs return uniformly-random short counts in `[1, op.buf.len)`; zero is reserved for "premature EOF" / "no progress".
- `tests/storage_writer_buggify_test.zig` — 4 BUGGIFY tests over the canonical 32-seed list (`tests/recheck_live_buggify_test.zig`-shape). Covers: (1) 5-file partial-init cleanup under `fallocate_error_probability = 0.3` — 26/32 seeds hit fault, errdefer cleanup of partially-opened files validated by testing.allocator; (2) fsync error storm at per-seed p ∈ [0.5, 1.0] — 32/32 hit, sync's `pending` counter consistent across error completions; (3) 3-span write/read mixed faults at per-seed p ∈ [0.1, 0.7] each — 26/32 hit, 40 total hits across write+read phases, multi-completion drain + `first_error` arithmetic validated; (4) writePiece/readPiece short-write/short-read loops at p=0.7 — 32/32 round-trip success, offset arithmetic correct under many resubmit iterations. Vacuous-pass guards on every test (`hits * 2 >= seeds.len`).
- `build.zig` — new `test-storage-writer-buggify` step (focused) + `test_step` dependency so the harness runs under `zig build test`.

**No production bug surfaced.** Confidence-builder outcome on top of the 2026-04-28 storage IO contract refactor (`progress-reports/2026-04-28-storage-rw-io-contract.md`) and the truncate-op landing (`progress-reports/2026-04-28-truncate-op.md`); both rounds had inline tests at p=1.0 in isolation, and the BUGGIFY harness now validates that mixed-probability fault densities don't break the same paths.

`zig fmt .`: clean. `zig build`: clean. `zig build test`: green across 5 consecutive runs. `zig build test-storage-writer-buggify`: 4/4 tests pass with meaningful BUGGIFY hit counts. Sentinel-injection smoke check (`try testing.expect(false)` in one assertion) caught by the runner with the correct test name; reverted. See `progress-reports/2026-04-29-piece-store-buggify-harness.md`.

Branch: `worktree-agent-a12dafe743dd4dff5`.
