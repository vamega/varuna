...
### IO Abstraction (Stage 2 complete)
...
- **`SimIO` (`src/io/sim_io.zig`)**: in-process simulation backend with min-heap pending queue, seeded `std.Random.DefaultPrng`, `FaultConfig` (per-op error probabilities + latency injection). Zero-alloc after init.
