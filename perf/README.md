...
| Scenario | Result |
|----------|--------|
| `seed_plaintext_burst --iterations=500 --scale=8` | before: `30384358 ns`, `27940324 ns`, `28669060 ns`; after: `12969659 ns`, `12485114 ns`; alloc calls `1001`, `276 KB` transient bytes |
...
- `seed_plaintext_burst` is now the production plaintext upload path. Refcounted piece buffers plus tracked vectored `sendmsg` improved wall-clock time by about `2.2x` to `2.4x` on this host and cut transient bytes from `65.6 MB` down to `276 KB`.
...
- The remaining cost in the production plaintext path is small-state allocation count. The current tracked vectored send allocates one backing block per batch so the payload pages stay borrowed instead of copied. Pooling that state is the next lever if seed upload allocation count still matters.
