...
- Extended the tracker HTTP executor with optional plain-HTTP keep-alive reuse and a small connection cache so the shared executor can reuse tracker sockets across announces and scrapes.
- Added real announce-path benchmarks so the production path can be compared directly: the former fresh-request path and `tracker_announce_executor`. See `src/perf/workloads.zig:893` and `src/perf/workloads.zig:941`.
