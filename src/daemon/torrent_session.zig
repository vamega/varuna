...
pub fn TorrentSessionOf(comptime IO: type) type {
    return struct {
        ...
        const PieceStore = storage.writer.PieceStoreOf(IO);
        ...
        const AsyncRecheck = EventLoop.AsyncRecheck;
        const AsyncMetadataFetch = EventLoop.AsyncMetadataFetch;
        ...
        pub fn integrateIntoEventLoop(self: *Self) bool {
        ...
        pub fn addPeersToEventLoop(self: *Self) bool {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        fn onRecheckComplete(recheck: *AsyncRecheck) void {
            const self: *Self = if (recheck.caller_ctx) |ctx|
        ...
        pub fn forceRecheckLive(self: *Self) !bool {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn dhtV2HashTruncated(self: *const Self) ?[20]u8 {
        ...
        pub fn dhtRegisterPeers(self: *const Self, engine: *DhtEngine) bool {
        ...
        pub fn dhtForceRequery(self: *const Self, engine: *DhtEngine) bool {
            if (self.is_private) return false;
            const v2_truncated = self.dhtV2HashTruncated();
        ...
        pub fn dhtAnnouncePeer(self: *const Self, engine: *DhtEngine) bool {
            if (self.is_private) return false;
            const v2_truncated = self.dhtV2HashTruncated();
        ...
        fn computeAnnounceJitterMs(self: *const Self) u64 {
        ...
        pub fn applyFilePriorities(self: *Self) bool {
        ...
        pub fn applySequentialMode(self: *Self) void {
            if (self.piece_tracker) |*pt| {
        ...
        pub fn setSuperSeeding(self: *Self, enabled: bool) void {
        ...
        fn getTagsString(self: *const Self) []const u8 {
        ...
        pub fn rebuildTagsString(self: *Self) void {
        ...
        fn isPartialSeed(self: *Self) bool {
            if (self.piece_tracker) |*pt| {
        ...
        fn statsState(self: *const Self, pieces_have: u32, partial_seed: bool) State {
        ...
        fn doStartBackground(self: *Self) !void {
            ...
            // backend instance just for the duration of init via
            // backend.initOneshotFor — that picks the right `init` shape for
            // whichever -Dio= backend the build selected. Cost: one
            // ring/epoll/kqueue setup + teardown per torrent (~tens of µs);
            // benefit: every disk I/O in the daemon is uniformly
            // contract-routed and BUGGIFY-injectable from sim tests.
            var init_io = try backend.initOneshotFor(IO, self.allocator);
            defer init_io.deinit();
            const store = try PieceStore.init(self.allocator, &self.session.?, &init_io);
            self.store = store;
        ...
        pub fn integrateSeedIntoEventLoop(self: *Self) bool {
            const sel = self.shared_event_loop orelse return false;
        ...
        pub fn checkSeedTransition(self: *Self) bool {
        ...
        fn submitTrackerJob(self: *Self, url: []const u8, host: []const u8, on_complete: TrackerExecutor.CompletionFn) !void {
        ...
        pub fn scheduleReannounce(self: *Self) !void {
        ...
        fn trySubmitUdpAnnounce(self: *Self, request: tracker.announce.Request) bool {
            return self.trySubmitUdpAnnounceWithCallback(request, udpAnnounceComplete);
        }

        fn trySubmitUdpAnnounceWithCallback(self: *Self, request: tracker.announce.Request, on_complete: UdpTrackerExecutor.CompletionFn) bool {
        ...
        fn udpAnnounceComplete(context: *anyopaque, result: UdpTrackerExecutor.RequestResult) void {
        ...
        pub fn maybeScrape(self: *Self) void {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        fn scheduleScrape(self: *Self) !void {
            const session = &(self.session orelse return error.MissingSession);
            const tracker_urls = try self.buildTrackerUrls(session);
            defer self.allocator.free(tracker_urls);
            if (tracker_urls.len == 0) return error.NoTrackers;
        ...
        fn trySubmitUdpScrape(self: *Self, announce_url: []const u8, info_hash: [20]u8) bool {
            const udp_mod = @import("../tracker/udp.zig");
        ...
        fn udpScrapeComplete(context: *anyopaque, result: UdpTrackerExecutor.RequestResult) void {
            const self: *Self = @ptrCast(@alignCast(context));
        ...
        /// metadata fetch.
        fn submitMagnetAnnounces(self: *Self) void {
        ...
        fn accumulateMagnetPeers(self: *Self, peers: []const tracker.announce.Peer) void {
            if (peers.len == 0) return;
        ...
        fn finishMagnetAnnounceJob(self: *Self) void {
            const remaining = self.announce_jobs_in_flight.fetchSub(1, .acq_rel) - 1;
            if (remaining == 0) {
                const tlog = std.log.scoped(.metadata_fetch);
        ...
        fn onMetadataFetchComplete(mf: *AsyncMetadataFetch) void {
            const self: *Self = if (mf.caller_ctx) |ctx|
                @ptrCast(@alignCast(ctx))
            else
                return;
            ...
            // Same one-shot backend pattern as doStartBackground above —
            // PieceStore.init now routes fallocate through the contract.
            var init_io_meta = backend.initOneshotFor(IO, self.allocator) catch |err| {
                self.state = .@"error";
            ...
            const store_result = PieceStore.init(self.allocator, &self.session.?, &init_io_meta);
        ...
        /// Build a minimal .torrent file wrapping the raw info dictionary.
        fn buildTorrentBytes(self: *Self, info_bytes: []const u8) ![]const u8 {
        ...
        fn buildTrackerUrls(self: *Self, session: *const session_mod.Session) ![]const []const u8 {
        ...
        pub fn addTrackerUrls(self: *Self, urls: []const []const u8) !void {
        ...
        pub fn removeTrackerUrls(self: *Self, urls: []const []const u8) !void {
            for (urls) |url| {
                if (url.len == 0) continue;
        ...
        pub fn editTrackerUrl(self: *Self, orig_url: []const u8, new_url: []const u8) !void {
        ...
        pub fn loadTrackerOverrides(self: *Self) void {
        ...
        fn persistTrackerOverride(self: *Self, url: []const u8, tier: u32, action: []const u8, orig_url: ?[]const u8) void {
            const db_path = self.resume_db_path orelse return;
            var db = ResumeDb.open(db_path) catch return;
            defer db.close();
        ...
        fn unpersistTrackerOverride(self: *Self, url: []const u8) void {
            const db_path = self.resume_db_path orelse return;
            var db = ResumeDb.open(db_path) catch return;
            defer db.close();
        ...
        pub fn persistNewCompletions(self: *Self) void {
        ...
        pub fn flushResume(self: *Self) void {
            if (self.resume_writer) |*rw| {
                rw.flush() catch {};
        ...
        fn detachFromSharedEventLoop(self: *Self) void {
            self.mutex.lock();
            defer self.mutex.unlock();
            ...
            if (sel.metadata_fetch) |mf| {
                if (mf.caller_ctx) |ctx| {
                    const mf_session: *Self = @ptrCast(@alignCast(ctx));
                    if (mf_session == self) {
        ...
        fn waitForBackgroundNetworkJobs(self: *Self) void {
...
pub const TorrentSession = TorrentSessionOf(backend.RealIO);

fn initTestEventLoop(allocator: std.mem.Allocator) !DefaultEventLoop {
    return DefaultEventLoop.initBare(allocator, 0);
}
