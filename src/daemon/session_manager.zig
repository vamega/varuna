...
const torrent_session_mod = @import("torrent_session.zig");
const DefaultTorrentSession = torrent_session_mod.TorrentSession;
const Stats = torrent_session_mod.Stats;
const TorrentState = torrent_session_mod.State;
...
const DefaultEventLoop = event_loop_mod.EventLoop;
const backend = @import("../io/backend.zig");

pub fn SessionManagerOf(comptime IO: type) type {
    return struct {
        const Self = @This();
        const TorrentSession = torrent_session_mod.TorrentSessionOf(IO);
        const EventLoop = event_loop_mod.EventLoopOf(IO);
        const TrackerExecutor = tracker_executor_mod.TrackerExecutorOf(IO);
        const UdpTrackerExecutor = udp_tracker_executor_mod.UdpTrackerExecutorOf(IO);
        ...
        pub fn loadCategoriesAndTags(self: *Self) void {
        ...
        fn initSmartBan(self: *Self) void {
        ...
        pub fn setDhtEnabled(self: *Self, enabled: bool) void {
        ...
        fn configureManagedSession(self: *Self, session: *TorrentSession) !void {
        ...
        fn registerSession(self: *Self, session: *TorrentSession) !*TorrentSession {
            self.mutex.lock();
        ...
        pub fn removeTorrent(self: *Self, hash: []const u8) !void {
        ...
        pub fn persistBanList(self: *Self) void {
        ...
        /// Pause a torrent.
        pub fn pauseTorrent(self: *Self, hash: []const u8) !void {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn resumeTorrent(self: *Self, hash: []const u8) !void {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn getAllStats(self: *Self, allocator: std.mem.Allocator) ![]Stats {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn getTransferInfo(self: *Self, allocator: std.mem.Allocator) !TransferInfo {
        ...
        pub fn getStats(self: *Self, hash: []const u8) !Stats {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn setSequentialDownload(self: *Self, hash: []const u8, enabled: bool) !void {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn setFilePriority(self: *Self, hash: []const u8, file_indices: []const u32, priority: u8) !void {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn setSuperSeeding(self: *Self, hash: []const u8, enabled: bool) !void {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn addTrackers(self: *Self, hash: []const u8, urls: []const []const u8) !void {
            self.mutex.lock();
        ...
        pub fn removeTrackers(self: *Self, hash: []const u8, urls: []const []const u8) !void {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn editTracker(self: *Self, hash: []const u8, orig_url: []const u8, new_url: []const u8) !void {
            self.mutex.lock();
            const session = self.sessions.get(hash) orelse {
                self.mutex.unlock();
                return error.TorrentNotFound;
        ...
        pub fn forceReannounce(self: *Self, hash: []const u8) !void {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        fn ensureTrackerExecutor(self: *Self) !*TrackerExecutor {
        ...
        fn ensureUdpTrackerExecutor(self: *Self) !*UdpTrackerExecutor {
        ...
        pub fn forceRecheck(self: *Self, hash: []const u8) !void {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn queueTopPrio(self: *Self, hash: []const u8) !void {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn queueBottomPrio(self: *Self, hash: []const u8) !void {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn queueIncreasePrio(self: *Self, hash: []const u8) !void {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn queueDecreasePrio(self: *Self, hash: []const u8) !void {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn runQueueEnforcement(self: *Self) void {
            self.mutex.lock();
            defer self.mutex.unlock();
            self.runQueueEnforcementLocked();
        }
        ...
        fn runQueueEnforcementLocked(self: *Self) void {
        ...
        fn persistQueuePositions(self: *Self) void {
            self.mutex.lock();
            defer self.mutex.unlock();
            self.persistQueuePositionsLocked();
        }
        ...
        fn persistQueuePositionsLocked(self: *Self) void {
            if (self.resume_db) |*db| {
        ...
        pub fn loadQueueState(self: *Self) void {
            if (self.resume_db) |*db| {
                self.queue_manager.loadFromDb(db);
            }
        }
        ...
        pub fn setTorrentCategory(self: *Self, hash: []const u8, category_name: []const u8) !void {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn addTorrentTags(self: *Self, hash: []const u8, tag_names: []const u8) !void {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn removeTorrentTags(self: *Self, hash: []const u8, tag_names: []const u8) !void {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn startMoveJob(self: *Self, hash: []const u8, new_path: []const u8) !MoveJobId {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn getMoveJobProgress(self: *Self, id: MoveJobId) !MoveJobProgress {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        /// `error.JobNotFound` if the id is unknown.
        pub fn cancelMoveJob(self: *Self, id: MoveJobId) !void {
            self.mutex.lock();
            defer self.mutex.unlock();
            const job = self.move_jobs.get(id) orelse return error.JobNotFound;
            job.requestCancel();
        ...
        pub fn commitMoveJob(self: *Self, id: MoveJobId) !void {
            self.mutex.lock();
            defer self.mutex.unlock();
            const job = self.move_jobs.get(id) orelse return error.JobNotFound;
        ...
        pub fn forgetMoveJob(self: *Self, id: MoveJobId) !void {
            self.mutex.lock();
            defer self.mutex.unlock();
            const job = self.move_jobs.get(id) orelse return error.JobNotFound;
            const p = job.progress();
        ...
        pub fn setTorrentDlLimit(self: *Self, hash: []const u8, limit: u64) !void {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        /// Persists to SQLite so the limit survives daemon restarts.
        pub fn setTorrentUlLimit(self: *Self, hash: []const u8, limit: u64) !void {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn setShareLimits(self: *Self, hash: []const u8, ratio_limit: f64, seeding_time_limit: i64) !void {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn checkShareLimits(self: *Self) u32 {
            self.mutex.lock();
        ...
        fn checkTorrentShareLimit(self: *const Self, session: *const TorrentSession, stats: Stats) ShareLimitAction {
        ...
        pub fn persistCompletionOn(self: *Self, info_hash: [20]u8, ratio_limit: f64, seeding_time_limit: i64, completion_on: i64) void {
            if (self.resume_db) |*db| {
        ...
        pub fn getConnDiagnostics(self: *Self, hash: []const u8) !ConnDiagnostics {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn getSessionFiles(self: *Self, allocator: std.mem.Allocator, hash: []const u8) ![]const FileInfo {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn getSessionTrackers(self: *Self, allocator: std.mem.Allocator, hash: []const u8) ![]const TrackerInfo {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn getSessionProperties(self: *Self, allocator: std.mem.Allocator, hash: []const u8) !PropertiesInfo {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn getWebSeedUrls(self: *Self, allocator: std.mem.Allocator, hash: []const u8) ![][]const u8 {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn setGlobalDlLimit(self: *Self, limit: u64) void {
            if (self.shared_event_loop) |el| {
        ...
        pub fn setGlobalUlLimit(self: *Self, limit: u64) void {
            if (self.shared_event_loop) |el| {
        ...
        pub fn renameTorrent(self: *Self, hash: []const u8, new_name: []const u8) !void {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn toggleSequentialDownload(self: *Self, hash: []const u8) !void {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn forceStartTorrent(self: *Self, hash: []const u8) !void {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn getPieceStates(self: *Self, allocator: std.mem.Allocator, hash: []const u8) ![]u8 {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn getPieceHashes(self: *Self, allocator: std.mem.Allocator, hash: []const u8) ![][]const u8 {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn addManualPeers(self: *Self, hash: []const u8, peers_str: []const u8) !void {
            self.mutex.lock();
            defer self.mutex.unlock();
        ...
        pub fn getTorrentPeers(self: *Self, allocator: std.mem.Allocator, hash: []const u8) ![]const PeerInfo {
            self.mutex.lock();
            defer self.mutex.unlock();
...
pub const SessionManager = SessionManagerOf(backend.RealIO);
...
test "checkTorrentShareLimit ratio enforcement" {
    ...
    var session = DefaultTorrentSession{
