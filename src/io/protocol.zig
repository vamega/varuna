...
pub fn processMessage(self: *EventLoop, slot: u16) void {
    ...
    switch (id) {
        ...
        2 => { // interested
            ...
            // but doing an immediate unchoke here makes the first piece fly
            // out in a sub-tick rather than waiting up to unchoke_interval_secs
            // for the next recalc. peer.mode records who dialed, not who
            // serves — both orientations should auto-unchoke on interest.
            if (peer.am_choking) {
        ...
        6 => { // request
            // Serve any peer we've unchoked. The peer.mode flag records who
            // opened the TCP connection (outbound = we connected; inbound =
            // they connected) — it does not indicate the role in the swarm.
            // When a seeder dials out (e.g. after the tracker hands it our
            // address), our peer record for the leecher has mode=outbound
            // but we are still the one with pieces to serve.
...
/// the download stalls.
fn maybeSendInterested(self: *EventLoop, slot: u16, tc: *const @import("types.zig").TorrentContext) void {
    const peer = &self.peers[slot];
...
/// Helper: send BITFIELD (if any pieces complete) then INTERESTED for an outbound
/// peer. Without this, an outbound-mode peer (e.g. a seeder that initiated the
/// connection after finding us via the tracker) never advertises its pieces,
/// stranding the remote side — which receives no BITFIELD and therefore never
/// becomes interested. Mirrors the inbound bitfield/unchoke flow but omits the
/// unchoke (outbound peers are the "leecher side" of a connection by role).
pub fn sendOutboundBitfieldThenInterested(self: *EventLoop, slot: u16) void {
    const peer = &self.peers[slot];
