...
test "resolve numeric ipv4 does not use cache" {
...
test "resolve numeric ipv6 does not use cache" {
    var resolver = DnsResolver.init();
    defer resolver.deinit(std.testing.allocator);
...
test "invalidate nonexistent key is no-op" {
    var resolver = DnsResolver.init();
    defer resolver.deinit(std.testing.allocator);
...
test "cache updates existing entry without duplication" {
    var resolver = DnsResolver.init();
    defer resolver.deinit(std.testing.allocator);
...
test "resolve localhost via real DNS" {
...
test "cache respects port override on hit" {
    var resolver = DnsResolver.init();
    defer resolver.deinit(std.testing.allocator);
