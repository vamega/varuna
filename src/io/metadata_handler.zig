...
///
/// Daemon callers continue to write `AsyncMetadataFetch` (the
/// `AsyncMetadataFetchOf(RealIO)` alias declared below). Sim tests
/// instantiate `AsyncMetadataFetchOf(SimIO)` directly so the same state
/// machine drives against `EventLoopOf(SimIO)` for fault-injection
/// harnesses.
///
/// Lifecycle:
...
pub fn AsyncMetadataFetchOf(comptime IO: type) type {
    return struct {
        const Self = @This();
        ...
        fn slotIdxFor(self: *const Self, slot: *const Slot) u8 {
        ...
        fn connectPeer(self: *Self, slot_idx: u8) void {
        ...
        fn onConnectComplete(self: *Self, slot: *Slot, slot_idx: u8, res: i32) void {
            if (res < 0) {
        ...
        fn onSendComplete(self: *Self, slot: *Slot, slot_idx: u8, res: i32) void {
            if (res <= 0) {
        ...
        fn onRecvComplete(self: *Self, slot: *Slot, slot_idx: u8, res: i32) void {
            if (res <= 0) {
        ...
        fn processHandshake(self: *Self, slot: *Slot, slot_idx: u8) void {
        ...
        fn processExtMessage(self: *Self, slot: *Slot, slot_idx: u8, want_ext_hs: bool) void {
            const recv_buf = slot.recv_buf orelse {
                self.releaseSlot(slot_idx);
                self.tryNextPeer(slot_idx);
                return;
        ...
        fn processExtHandshake(self: *Self, slot: *Slot, slot_idx: u8, payload: []const u8) void {
        ...
        fn processPieceResponse(self: *Self, slot: *Slot, slot_idx: u8, payload: []const u8) void {
        ...
        fn requestNextPiece(self: *Self, slot: *Slot, slot_idx: u8) void {
        ...
        fn sendReject(_: *Self, _: *Slot, _: u8, _: u32) void {
        ...
        fn verifyAndComplete(self: *Self, slot_idx: u8) void {
        ...
        fn finish(self: *Self, success: bool) void {
            if (self.done) return;
            self.done = true;
        ...
        fn releaseSlot(self: *Self, slot_idx: u8) void {
            const slot = &self.slots[slot_idx];
        ...
        fn tryNextPeer(self: *Self, slot_idx: u8) void {
            if (self.done) return;
        ...
        fn resetForNextMessage(self: *Self, slot: *Slot, slot_idx: u8) void {
            slot.recv_len = 0;
            slot.recv_expected = 4;
            slot.reading_msg_header = true;
            self.submitRecv(slot, slot_idx);
        ...
        fn submitSend(self: *Self, slot: *Slot, slot_idx: u8) void {
            const send_buf = slot.send_buf orelse {
                self.releaseSlot(slot_idx);
                self.tryNextPeer(slot_idx);
                return;
        ...
        fn submitRecv(self: *Self, slot: *Slot, slot_idx: u8) void {
            const recv_buf = slot.recv_buf orelse {
                self.releaseSlot(slot_idx);
                self.tryNextPeer(slot_idx);
                return;
...
/// Daemon-side alias: the `RealIO` instantiation of the metadata fetch
/// state machine. Sim tests can instantiate `AsyncMetadataFetchOf(SimIO)`
/// directly; production code references this alias unchanged.
pub const AsyncMetadataFetch = AsyncMetadataFetchOf(RealIO);
