...
fn runDaemonWithBackend(
    ...
    while (!varuna.io.signal.isShutdownRequested()) {
        ...
        const has_io = is_draining or session_manager.hasActiveMoveJobs() or session_manager.hasActiveDeleteJobs() or shared_el.peer_count > 0 or shared_el.listen_fd >= 0 or shared_el.udp_fd >= 0 or shared_el.rechecks.items.len > 0 or shared_el.metadata_fetch != null or shared_el.timer_pending;
        if (has_io) {
            session_manager.tickMoveJobs();
            session_manager.tickDeleteJobs();
            shared_el.submitTimeout(100 * std.time.ns_per_ms) catch {};
