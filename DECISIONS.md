...
- Extend the HTTP tracker path with optional plain-HTTP keep-alive reuse, owned by the executor's client. HTTPS and UDP tracker requests still use one-shot transports.
- Add a real benchmark surface for the production tracker path: the former fresh-request path versus `tracker_announce_executor`.
...
- Using the main peer `EventLoop` ring safely would require turning tracker HTTP/HTTPS/UDP flows into proper asynchronous state machines. The former synchronous HTTP API would stall peer processing if called directly on the shared peer loop thread.
